...
// ✅ Define this at the top (outside the function) so it's globally available in this file
...
function CustomerDashboard() {
  ...
  const fetchOrder = async (id) => {
    try {
      // ✅ Fixed: Removed 'git add .' from inside the code and used API_BASE_URL
  ...
  return (
    ...
    >
      ...
      {/* MODALS (Game & Rating) ... kept as per your existing code ... */}
      {showGameModal && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}>
          <div style={{ width: "90%", maxWidth: "1100px", borderRadius: "25px", position: "relative" }}>
            <button onClick={() => setShowGameModal(false)} style={{ position: "absolute", top: "-40px", right: "0", border: "none", background: "#ff6ec7", color: "white", padding: "8px 16px", borderRadius: "10px", cursor: "pointer" }}>Close ✕</button>
            <GamesHub />
          </div>
        </div>
      )}
      ...
      {showRatingPopup && orderDetails && (
        <div style={{ position: "fixed", top: 0, left: 0, width: "100%", height: "100%", background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
          <div style={{ background: "white", padding: "30px", borderRadius: "20px", width: "400px", textAlign: "center" }}>
            <h3>⭐ Rate Your Order</h3>
            {orderDetails.items?.map((dish, index) => (
              <div key={index} style={{ marginTop: "15px" }}>
                <p>{dish.name}</p>
                {[1, 2, 3, 4, 5].map((star) => (
                  <span key={star} onClick={() => handleRating(dish.name, star)} onMouseEnter={() => setHoverRating((prev) => ({ ...prev, [dish.name]: star }))} onMouseLeave={() => setHoverRating((prev) => ({ ...prev, [dish.name]: 0 }))} style={{ cursor: "pointer", fontSize: "28px", margin: "5px", transition: "0.2s", color: (hoverRating[dish.name] || ratings[dish.name]) >= star ? "#ffc107" : "#ccc" }}>★</span>
                ))}
              </div>
            ))}
            <button onClick={submitRatings} style={{ marginTop: "20px", padding: "10px 20px", border: "none", borderRadius: "10px", background: "#845ec2", color: "white", cursor: "pointer" }}>Submit Rating</button>
          </div>
        </div>
      )}
    </div>
